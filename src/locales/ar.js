export default {
    intro: {
        title_pre: "صائد",
        title_post: "الصنابير",
        step1: "مسافة 3 أمتار",
        step1_bold: "3 أمتار",
        step2: "التقط صورة",
        step2_bold: "صورة",
        step3: "أضف البيانات",
        step3_bold: "بيانات",
        step4: "رفع لـ OSM",
        step4_bold: "رفع",
        start_btn: "ابدأ",
        login_osm: "تسجيل دخول OSM",
        login_connected: "متصل",
        info_legal: "معلومات قانونية",
        info_impressum: "بيانات قانونية",
        info_data: "البيانات والخصوصية",
        info_data_text: "تُعالَج الصور مؤقتًا ومحليًا فقط داخل المتصفح، ولا يتم رفعها أو تحليلها. يُحدَّد موقع GPS بشكل منفصل من خلال إذن الوصول إلى الموقع. لا تُرسَل الإحداثيات وبيانات صنبور الإطفاء إلى OpenStreetMap إلا عند بدء الرفع بصورة صريحة. لا تُحفَظ نسخة من الصورة تتضمن بيانات GPS EXIF إلا عند اختيار المشاركة أو التنزيل بصورة صريحة. عند استخدام خدمات الخرائط الخارجية وOverpass وNominatim وOpenStreetMap، يُرسَل أيضًا عنوان IP الخاص بالجهاز لأسباب تقنية.",
        info_license: "الترخيص والكود",
        disclaimer_text: "لا تستخدم في حالات الطوارئ. لا ضمانات.\nالبدء يعني الموافقة على استخدام البيانات.",
        info_github: "عرض على GitHub",
        lang_btn_aria: "اللغة / Language"
    },
    pwa: {
        update_available: "يتوفر إصدار جديد من التطبيق.",
        update_now: "التحديث الآن"
    },
    hydrant_data: {
        loading: "جارٍ تحميل بيانات صنابير الإطفاء …",
        slow: "يستغرق طلب البيانات وقتًا أطول",
        current: "بيانات صنابير الإطفاء محدّثة",
        unavailable: "بيانات صنابير الإطفاء غير متاحة حاليًا",
        stale: "البيانات الحالية – فشل التحديث"
    },
    settings: {
        title: "الإعدادات",
        map_style: "نمط الخريطة",
        account: "حساب OSM",
        connect_btn: "الاتصال بـ OSM",
        disconnect_btn: "تسجيل الخروج",
        app_reset: "إعادة تعيين التطبيق",
        reset_btn: "إعادة تعيين ومسح الذاكرة المؤقتة",
        back_btn: "رجوع",
        legal_link: "الحواشي القانونية",
        history_btn: "السجل"
    },
    gamification: {
        rank_progress: "باقي {count} للوصول لـ {rank}!",
        rank_max: "أنت أسطورة!",
        legend: "الرتب والشارات",
        list_title: "كل الرتب",
        from_hydrants: "من {count} صنابير",
        level_title: "رتبة صائد الصنابير"
    },
    history: {
        title: "السجل",
        btn_label: "السجل",
        loading: "جار التحميل...",
        error: "خطأ في التحميل",
        no_data: "لم يتم العثور على تغييرات.",
        not_logged_in: "غير مسجل الدخول.",
        back_btn: "رجوع"
    },


    confirm: {
        title: "بيانات الصنبور",
        title_edit: "تعديل الصنبور",
        preview_alt: "الصورة الملتقطة",
        back_btn_aria: "العودة للكاميرا",
        back_btn_label: "رجوع",
        cancel_btn: "إلغاء",
        retry_gps_aria: "إعادة تحديد الموقع GPS",
        delete_btn: "حذف",
        delete_confirm: "هل أنت متأكد من حذف الصنبور؟",
        delete_confirm_cistern: "هل أنت متأكد من حذف الخزان؟",
        delete_confirm_suction: "هل أنت متأكد من حذف نقطة الشفط؟",
        water_source_label: "مصدر المياه",
        water_source_default: "غير محدد",
        water_source_main: "شبكة رئيسية",
        water_source_groundwater: "مياه جوفية",
        water_source_pond: "بركة",
        water_source_lake: "بحيرة",
        water_source_river: "نهر",
        water_source_reservoir: "خزان/حوض",
        save_btn: "حفظ",
        update_btn: "تحديث OSM",
        position_adjustable: "الموقع قابل للتعديل",
        position_moved: "تم تعديل الموقع",
        fixed_map: "قابل للتحريك (الخريطة ثابتة)",
        click_to_save: "انقر مرة أخرى للحفظ",
        type_label: "النوع",
        position_label: "الموقع",
        details_label: "تفاصيل (اختياري)",
        upload_btn: "رفع إلى OSM",
        uploading: "جار الرفع...",
        success: "تم الرفع بنجاح!",
        error: "خطأ في الرفع",
        types: {
            underground: "تحت الأرض",
            pillar: "فوق الأرض",
            wall: "جداري",
            suction: "نقطة شفط",
            cistern: "خزان",
            dry_hydrant: "صنبور جاف",




        },
        locations: {
            sidewalk: "رصيف",
            street: "شارع",
            green: "منطقة خضراء",
            parking: "موقف سيارات",
            none: "غير محدد"
        },
        colors: {
            black: "أسود",
            grey: "رمادي",
            blue: "أزرق",
            red: "أحمر",
            yellow: "أصفر",
            green: "أخضر",
            white: "أبيض"
        },
        diameter_label: "القطر / الحجم",
        diameter_none: "غير محدد",
        sign_label: "علامة للصنبور",
        sign_options: {
            yes: "نعم",
            no: "لا",
            unknown: "غير معروف"
        },
        number_label: "رقم / مرجع",
        number_placeholder: "مثال 1234",
        notes_label: "ملاحظات",
        notes_placeholder: "..."
        ,
        share_title: "Hydranten Jäger",
        share_text: "Hydrant captured with Hydranten Jäger",
        diameter_placeholder: "e.g. 80, 100",
        volume_placeholder: "e.g. 100"
    },
    general: {
        loading: "جار التحميل...",
        error: "خطأ",
        success: "نجاح",
        close: "إغلاق",
        done: "تم",
        cancel: "إلغاء",
        save: "حفظ",
        delete: "حذف",
        back: "رجوع",
        retry: "إعادة المحاولة",
        saving: "Saving..."
    },
    messages: {
        locating_position: "تحديد الموقع...",
        please_login: "يرجى تسجيل الدخول للتحرير.",
        loading_hydrant: "تحميل صنبور #{id}...",
        saving_data: "جار الحفظ...",
        deleting_data: "جار الحذف...",
        internal_error_reload: "خطأ داخلي. يرجى إعادة التحميل.",
        camera_error: "خطأ في الكاميرا: {error}",
        gps_update_failed: "فشل تحديث GPS: {error}",
        no_osm_credentials: "لا توجد بيانات دخول OSM.",
        to_settings: "الإعدادات",
        uploading: "جار الرفع...",
        upload_successful: "تم الرفع بنجاح!",
        upload_wait: "جار الرفع... ⏳",
        node_id: "معرف العقدة",
        changeset: "مجموعة التغييرات",
        upload_failed: "فشل الرفع",
        verifying_login: "التحقق من الدخول...",
        back_to_start: "العودة للبداية"
    },
    camera: {
        permission: "يجب السماح للكاميرا",
        retake: "إعادة",
        use_photo: "استخدام",
        back_btn_aria: "عودة للكاميرا",
        back_btn_label: "رجوع",
        capture_btn_aria: "التقاط صورة",
        error_title: "الكاميرا غير متاحة",
        error_access: "لا يوجد وصول",
        compass_label: "بوصلة",
        gps_searching: "GPS: جارٍ البحث..."
    },
    error: {
        load_failed: "فشل التحميل",
        node_deleted: "هذا الصنبور لم يعد موجوداً.",
        oops: "عفواً!",
        back_to_map: "عودة للخريطة",
        gps_failed: "تعذر تحديد الموقع. تحقق من GPS.",
        gps_unavailable: "GPS غير متاح. الخريطة الافتراضية.",
        edit_function_missing: "خطأ داخلي: ميزة التعديل مفقودة.",
        network_error: "فشل التحميل (شبكة/API)",
        back_unavailable: "خطأ: الرجوع غير متاح."
    },
    legal: {
        important_header: "⚠️ هام:",
        tmg_header: "معلومات قانونية:"
    },
    upload_log: {
        locating_nominatim: "تحديد اسم الموقع (Nominatim)...",
        unknown_location: "غير معروف",
        location_fallback: "مكان",
        nominatim_error: "خطأ Nominatim: {status}",
        creating_changeset: "إنشاء مجموعة تغييرات...",
        uploading_hydrant: "جاري رفع الصنبور...",
        starting_update: "بدء التحديث...",
        update_conflict: "تضارب! قام شخص ما بالتعديل للتو.",
        update_success: "تم التحديث بنجاح!",
        deleting_node: "جاري حذف {type} #{id}...",
        delete_conflict: "تضارب! فشل الحذف.",
        already_deleted: "تم الحذف بالفعل.",
        delete_success: "تم الحذف!"
    }
};
